...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    if (!dbReady) {
      setError("המערכת לא מחוברת למסד נתונים. נא לבדוק את הגדרות ה-Firebase.");
      return;
    }
    ...
    try {
      if (mode === 'signup') {
        ...
        if (res.success) {
          setSuccessMsg("המרחב נוצר בהצלחה ב-Cloud! מתחבר...");
      ...
      setError(`שגיאת שרת: ${err.message}`);
    }
    setLoading(false);
  };
      ...
      <div className="glass-card w-full max-w-lg rounded-[3.5rem] p-12 border-white/10 shadow-[0_0_80px_rgba(45,212,191,0.1)] relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-black text-white mt-6 tracking-tight italic">
            {mode === 'signup' ? 'Create Cloud Workspace' : mode === 'login' ? 'Manager Login' : 'Team Pulse Access'}
          </h2>
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className={`text-[9px] font-black uppercase tracking-widest ${dbReady ? 'text-slate-500' : 'text-red-400'}`}>
              {dbReady ? 'Firebase Firestore Active' : 'Database Disconnected'}
            </span>
          </div>
        </div>

        {!dbReady && (
          <div className="mb-6 bg-red-500/10 border border-red-500/20 p-4 rounded-2xl text-red-400 text-xs font-bold text-center leading-relaxed">
            ⚠️ שים לב: מפתחות ה-Firebase חסרים. <br/>
            המערכת דורשת VITE_FIREBASE_API_KEY וכו'.
          </div>
        )}
        ...
        <form onSubmit={handleAction} className="space-y-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{mode === 'signup' ? 'צור מרחב ב-Firebase' : mode === 'login' ? 'כניסה למרחב הענן' : 'כניסה ל-Pulse'}</span>
        ...
        <p className="mt-8 text-center text-slate-600 text-[9px] font-bold uppercase tracking-[0.3em] leading-relaxed">
          {mode === 'signup' 
            ? 'By creating a workspace, you define a secure boundary in Firestore.' 
            : 'All data is strictly stored and synchronized via Firebase.'}
        </p>
      </div>
    </div>
  );
